...
n_qubit = 20
n_points = 10
d_values = np.linspace(10, 5000, num=n_points).astype(int).tolist()
...
def plot():
    ...
    plt.errorbar(d_unique, means_uniform, yerr=stds_uniform, fmt="o-",
                 label="Uniform", color="#2D2F92")
    plt.errorbar(d_unique, means_single, yerr=stds_single, fmt="s--",
                 label="Single Rotations w/ mergings", color="#DC3977")
    plt.xlabel("d (sparsity)")
