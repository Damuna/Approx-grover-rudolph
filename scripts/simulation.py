...
# Fixed sparsity for the M-sweep plot
D_fixed = 1e-4
...
def compute_values_M_sweep(
    ...
    # Approximate mergings with variable M
    approx_angles = copy.deepcopy(exact_angles)
    ordering_geometric_series(
        approx_angles,
        min_overlap,
    ...
    return (
        f"{min_overlap}\t{sparsity}\t{M_value}\t"
        f"{num_gates_approx}\t{num_gates_exact}\n"
    )
...
def _grouped_errorbar_by_d(x_values, d_data, y_values, ylabel, filename):
    ...
    for i, fixed_d_val in enumerate(d_values):
        ...
        means = np.array([np.mean(y[x == ux]) for ux in unique_x])
        stds = np.array([np.std(y[x == ux]) for ux in unique_x])
